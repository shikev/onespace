...
class Auth extends CI_Controller
{
	...
	function testa() {
		echo '<pre>';
		var_dump(password_hash("poopybutthole", PASSWORD_BCRYPT));
		echo '</pre>';
		echo '<pre>';
		var_dump(password_hash("poopybutthole", PASSWORD_BCRYPT));
		echo '</pre>';
			...
			if ($this->form_validation->run()) {								// validation ok
				if (!is_null($data = $this->tank_auth->create_user(
					...
					if ($email_activation) {									// send "activate" email
						...
						$this->tank_auth->login($this->form_validation->set_value('username'), $this->form_validation->set_value('password'), 1, 1, 1);
						unset($data['password']);
						redirect('/userpage/manage');;

						//$this->load->view('content/redirect.php', $message);
	...
	function forgot_password()
	{
		...
		if ($this->tank_auth->is_logged_in()) {									// logged in
			...
			if(!$preventDefault){
				$headerData['pageTitle'] = " | Forgot Password";
	...
	function reset_password()
	{
		...
		if ($this->form_validation->run()) {								// validation ok
			if (!is_null($data = $this->tank_auth->reset_password(
				...
				$message['message'] = 'Your password has been successfully reset. You will be redirected to the login page in three seconds...';
				$this->load->view('content/redirect.php', $message);
		...
		$headerData['pageTitle'] = " | Reset Password";
		...
		if (!$this->tank_auth->is_logged_in()) {								// not logged in or not activated
			...
			if ($this->form_validation->run()) {								// validation ok
				if ($this->tank_auth->change_password(
						...
						$preventDefault = true;
						$headerData['message'] = 'true';
						$headerData['pageTitle'] = " | Change Password";
